...
- **9 Jogos Integrados:**
  ...
  - **Snake Retro** (Pontuação em tela, canvas fluido). Sistema de controles com **Fila de Inputs (Queue)** perfeita anti-ghosting e detecção de Swipes (Arrastar).
  - **Memória Master** (Animações de cartas em CSS 3D e múltiplos níveis de Grade).
  - **Flappy Bird** (Física de gravidade fluida, colisão customizada e cenário progressivo).
  - **Mines** 💣 (Mecânica de Cassino: Seleção estocástica de bombas, grade interativa e acúmulo de Multiplicador variável com cashouts).
  - **Crash Turbo** 🚀 (Gráfico multiplicador que infla progressivamente até o "crash" point. Teste seu reflexo para o cashout manual ou limite automático).
  ...
  - **Caça-Níqueis (Slots)** 🎰 (Rolo giratório com pesos probabilísticos ajustados, multiplicando aposta em até 100x no Jackpot).
- **Sistema de Banca Realista:** Se o jogador fizer uma aposta que zere sua banca (All-in) e perder a rodada, receberá instantaneamente um bônus de recuperação de 10.000 SQC.
- **Modo Cassino Autenticado:** Proteções de front-end para exibição de balanço monetário e seções com apostas estritamente liberadas mediante sessão JWT ativa.
